...
#
# Model info:
#   File: gemma-4-E4B-OBLITERATED-Q8_0.gguf (7.5GB, near-lossless)
#   mmproj: mmproj-gemma-4-E4B-OBLITERATED-F16.gguf (945MB, required for vision/audio)
#   Source: https://huggingface.co/rdhorner/gemma-4-E4B-it-OBLITERATED-GGUF
#
# Distributed inference via VCN-LUPINE + fbdev:
#   Model layers split across collective compute resources:
#   - RTX 4070 (qfox-1) via CUDA/LUPINE          — VRAM layers
#   - AMD VCN (steamdeck) via VAAPI               — VCN-accelerated
#   - ARM CPU (neon-64gb, 18 cores)               — CPU fallback
#   - fbdev DMA  (nixos, /dev/fb0)                — framebuffer backplane
#   - virtio-net (nixos, racknerd)                — packet-as-computation
#   All nodes participate via the compute fabric.
#   Garage S3 serves the model to any requesting node.

set -euo pipefail

MODEL_PATH="${1:-/home/allaun/Downloads/gemma-4-E4B-OBLITERATED-Q8_0.gguf}"
...
echo "  GARAGE_ENDPOINTS=\"http://100.88.57.96:3900,http://100.110.163.82:3900,http://100.102.173.61:3900,http://100.64.19.78:3900,http://100.80.39.40:3900\""
